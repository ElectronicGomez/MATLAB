...
%Controlabilidad: se puede diseñar un controlador,es llevar un estado cualquiera de cualier valor a cualkier otro valor
CC = [B A*B A*A*B]
controlabilidad = rank(CC)
% 
%Observabilidad: Puedes ver los estados(capacidad de estimar) atravez de
%las salidas teniendo uno o poco sensores, se puede diseñar un observador
%de estados
OO = [C; C*A; C*A*A]
...
%pdc = [-5+40i -5-40i -5+40i -5-40i]
pdc = -5*ones(4,1); %pdc = -10*ones(4,1); menor polo se mueve a la derecha el eje x
Km = acker(Am,Bm,pdc)
K = Km(1,1:3);
kI= -Km(1,4);
% 
